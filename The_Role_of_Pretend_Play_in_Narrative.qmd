...
pretend_data_H8 <- read.csv("~/KristenWorkingDirectory/Play_Narrative/PN_Datasets/Pretend_Play/CSVs_of_Combined_Data_PP/pretend_data_H8.csv")
...
Analyses of gesture production during pretend play at 59 months revealed differences between typically developing (TD) children and children with pre- or perinatal unilateral brain injury (PL), though these differences did not reach statistical significance (see Table 1). Children with `r unique(child_summary$GroupStatus)[child_summary$GroupStatus == "BI"]` produced more total gestures during pretend play episodes (M = `r round(mean(child_summary$total_gestures[child_summary$GroupStatus == "BI"]), 2)`, SD = `r round(sd(child_summary$total_gestures[child_summary$GroupStatus == "BI"]), 2)`) compared to TD children (M = `r round(mean(child_summary$total_gestures[child_summary$GroupStatus == "TD"]), 2)`, SD = `r round(sd(child_summary$total_gestures[child_summary$GroupStatus == "TD"]), 2)`), though this difference was `r if(0.194 < 0.05) {"statistically significant"} else {"not statistically significant"}`, F(1, 21) = 1.80, p = .194, η² = .079. This medium effect size suggests that approximately 7.9% of the variance in total gesture production is explained by group status.
...
When examining the proportion of gestures that were representational, TD children had a higher proportion (M = 16.23%, SD = 24.75%) compared to children with PL (M = 3.03%, SD = 7.42%), F(1, 15) = 1.585, p = .227, η² = .096. Similarly, TD children had a higher proportion of iconic gestures (M = 13.20%, SD = 25.33%) compared to children with PL (M = 3.03%, SD = 7.42%), F(1, 15) = 0.900, p = .358, η² = .057. These findings indicate that while children with PL may gesture more overall, a smaller proportion of these gestures serve representational functions.
...
narrative_scores <- read_csv("~/KristenWorkingDirectory/Play_Narrative/PN_Datasets/Narrative/CSVs_of_Combined_Data_N/Narrative_Results_Final.csv")
