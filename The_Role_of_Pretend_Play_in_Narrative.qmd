...
Analyses of gesture production during pretend play at 59 months revealed differences between typically developing (TD) children and children with pre- or perinatal unilateral brain injury (PL), though these differences did not reach statistical significance (see [Figure 1](#PP_Gest_boxplot_props) . Children with brain injury produced more total gestures during pretend play episodes (M = `r round(mean(child_summary$total_gestures[child_summary$GroupStatus == "BI"]), 2)`, SD = `r round(sd(child_summary$total_gestures[child_summary$GroupStatus == "BI"]), 2)`) compared to TD children (M = `r round(mean(child_summary$total_gestures[child_summary$GroupStatus == "TD"]), 2)`, SD = `r round(sd(child_summary$total_gestures[child_summary$GroupStatus == "TD"]), 2)`), though this difference was `r if(0.194 < 0.05) {"statistically significant"} else {"not statistically significant"}`, F(1, 21) = 1.80, p = .194, η² = .079. This medium effect size suggests that approximately 7.9% of the variance in total gesture production is explained by group status.
