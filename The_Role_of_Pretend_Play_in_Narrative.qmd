...
Play_Narrative_Demographics <- read.csv("~/KristenWorkingDirectory/Play_Narrative/PN_Datasets/Narrative/CSVs_of_Combined_Data_N/Play_Narrative_Demographics.csv")
...
P3_neurological_data <- read.csv("~/KristenWorkingDirectory/Play_Narrative/PN_Datasets/Narrative/CSVs_of_Combined_Data_N/P3_neurological_data.csv")
...
Children's narrative abilities were assessed using the Stein narrative task, which presents children with story stems and asks them to create their own stories. Three stems were used: one about a fox living near a forest, another about a girl living near the ocean, and a third about a boy with many toys. Narrative organization at the global level is assessed using the model of story complexity by @steinn.l.AnalysisStoryComprehension1979, which defines a “good” story as one organized around a protagonist’ goal-directed action. These narratives are built out of four organizing features: 1) an animate protagonist, 2) temporal structure, 3) causal structure and 4) goal-direction action. The presence of each successive feature in this list is contingent upon the presence of earlier features, and stories with more features are considered to be more complex than stories with fewer features. Based on analyses of these features, each child's story was placed into one of the following nine categories: 

* **No structure (NS)** is a narrative that includes only one clause with or without a verb. 

* **Descriptive story (DS)** is a narrative that includes a series of clauses that do not report actions; even if it does report actions, the actions are not constrained by temporality.

* **Action story (AC)** is a narrative that includes actions *with a temporal order* (events follow one another in time), but story events are not causally organized.

* **Reactive story (RS)** is a narrative that includes actions that are causally organized, (events in one clause cause the actions in the following clause).

* **Incomplete goal-based story (IGB)** is a narrative that contains a goal statement, but no attempt or outcome following the goal.

* **Incomplete goal-based with attempt (IGBA)** is a narrative that contains a goal statement, and attempt(s), but not outcome following the goal.

* **Complete goal-based story with one episode implicit goal (GB1-IG)** is a narrative that includes not only temporal and causal structure, but also an *implicitly*-stated goal of the protagonist, *one* series of attempts to achieve the goal, and an outcome of these attempts.

* **Complete goal-based story with one episode explicit goal (GB1-IG)** is a narrative that includes not only temporal and causal structure, but also an *explicitly*-stated goal of the protagonist, *one* series of attempts to achieve the goal, and an outcome of these attempts.

* **Complete goal-based story with multiple episodes implicit goal (GBM-IG)** is a narrative that includes *multiple* goal-attempt-outcome sequences. Multiple episode stories include more than one series of attempts, and frequently include more than one *implicitly*-stated goal. 

* **Complete goal-based story with multiple episodes explicit goal (GBM-EG)** is a narrative includes *multiple* goal-attempt-outcome sequences. At least one of the goals is stated *explicitly*.
