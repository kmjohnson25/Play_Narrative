...
pretend_data_H7 <- read.csv(here("PN_Datasets", "Pretend_Play", "CSVs_of_Combined_Data_PP","pretend_data_H7.csv"))
...
pretend_data_H7 <- child_summary_H7 %>%
...
write_csv(pretend_data_H7, here("PN_DataAnalysis", "PP_Narrative_Analysis", "pretend_data_H7.csv"))

```

```{r}
# Create binary variable for representational gesture presence
pretend_data_H7 <- pretend_data_H7 %>%
  mutate(rep_gesture_present = ifelse(rep_gestures > 0, 1, 0))

# Check distributions
hist(pretend_data_H7$total_gestures)
hist(pretend_data_H7$rep_gestures)
table(pretend_data_H7$rep_gesture_present, pretend_data_H7$GroupStatus)

# Check correlation between total gestures and representational gestures
cor.test(pretend_data_H7$total_gestures, pretend_data_H7$rep_gestures)
```

```{r}
# 2. Data visualization
# Boxplot of total gestures by group
ggplot(pretend_data_H7, aes(x = GroupStatus, y = total_gestures, fill = GroupStatus)) +
  geom_boxplot() +
  theme_minimal() +
...
ggplot(pretend_data_H7, aes(x = GroupStatus, fill = factor(rep_gesture_present))) +
...
# MERGE IT
completely_merged_data_H7 <- left_join(pretend_data_H7, narrative_data, by = "participant_id") %>%
