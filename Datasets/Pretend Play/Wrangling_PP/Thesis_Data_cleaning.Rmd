...
directory_path <- '/Users/kristenjohnson/Desktop/University_of_Chicago/Thesis/Data/decontextualized/08/P2_H8'
...
load_and_bind_files <- function(files) {
  ...
  all_data <- lapply(files, function(file) {
    ...
    # For example, in "022.10_Final.xlsx":
    #   - ([0-9]+) matches "022"
    #   - \\. matches the period
    #   - ([0-9]+) matches "10"
    pattern <- "^([0-9]+)\\.([0-9]+)"
    ...
    if (length(extracted) > 0 && length(extracted[[1]]) >= 3) {
      # extracted[[1]][1] is the entire match,
      # extracted[[1]][2] is the participant ID,
      # extracted[[1]][3] is the session number.
      participant_id <- as.numeric(extracted[[1]][2])
      session <- as.numeric(extracted[[1]][3])
    } else {
    ...
    # Ensure 'p_word_num is a cahracter to prevent type mismatch
...
How we create a column that calculates words per utterance for each row with an utterance:
```{r}
...
# change the group_timepoint_item being coded
write.csv(combined_dataset, "/Users/kristenjohnson/KristenWorkingDirectory/Narrative Data/CSVs_of_Combined_Data/P2_H8_cwpu.csv", row.names = FALSE)

```

How do get descriptive statistics for ALL Gestures:
```{r}
# Load necessary libraries
...
Here is how you get the Mean and Standard deviation of ALL gestures 
```{r, echo=FALSE}
...
How to quickly get descriptive for iconic gestures: 

If you'd like to inspect a df that only contains the instances of iconic gestures:
...
if you're interested in inspecting how often individual kids do iconic gestures: 
```{r}
...
Now, if you want some summary statistics across participants, you can do this: 
```{r}
...
```
Here is how we calculate instances of Pretend Play:
```{r}
