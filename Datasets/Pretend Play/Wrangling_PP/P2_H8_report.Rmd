...
# Typically Developing Children during Home Visit 8
### N= 16; Age in months: M = 42
```{r, echo=FALSE}
#library(dplyr)
...
P2_H8_ALL <- read.csv('/Users/kristenjohnson/KristenWorkingDirectory/Narrative Data/CSVs_of_Combined_Data/P2_H8_ALL.csv')
...
```{r, echo=FALSE}
# Filter for rows where c_pret is 1, then count occurrences of c-form
PP_gest_rep_overlap <- P2_H8_ALL %>%
  filter(c_pret == 1 & 
         str_detect(c_form,"iconic|point-iconic-flip|nod\\+palm\\+palm-iconic|iconic\\s-\\siconic|iconic\\?|iconic-iconic-iconic-iconic|nod\\+iconic|iconic~|iconic-iconic|metaphoric|demo"))
...
# Filter for rows where c_pret is 1, then count occurrences of c-form
PP_gest_icon_overlap <- P2_H8_ALL %>%
  filter(c_pret == 1 & 
         str_detect(c_form,"iconic|point-iconic-flip|nod\\+palm\\+palm-iconic|iconic\\s-\\siconic|iconic\\?|iconic-iconic-iconic-iconic|nod\\+iconic|iconic~|iconic-iconic")) # be sure to adjust specific words for each new dataset
